...
public class ERule implements Serializable {
  ...
  public ERule(int label, int correct, int covers) {
    this(label);
  ...
  public ERule copy() {
    ERule result = new ERule(this.label);
  ...
  public void updateWith(EMaxIndex maxIndex) {
  ...
  public void updateWith(EarlyMax eMax) {
    assert this.label == eMax.getLabel();
    this.correct = eMax.getBestCorrect();
    this.covers = eMax.getBestCover();
    this.errors = covers - correct;
  }
  ...
  public String toString(Instances data, int maxDigits) {
    ...
    String pattern = "( " + EarlyUtils.formatIntPattern(maxDigits) + " , %.2f ) ";
