...
/**
 * <!-- globalinfo-start --> VaCfsEval :<br/>
 * <br/>
 * Combine Va rank evaluator with teh CFS subset evaluator
 * Add to the CFS evaluator a filter function that drops from the best subset the attributes which did not pass the vThreshold
 * <p>
 * * This code is based on SubsetEvaluator.java and "Correlation-based Feature Subset Selection for Machine Learning"
 * authored by Mark Hall (mhall@cs.waikato.ac.nz)
 * * <p/>
 * <!-- globalinfo-end -->
 * <p>
 ...
 * @author Fadi Thabtah (f.thabtah@gmail.com)
...
public class VaCfsEval extends ASEvaluation implements SubsetEvaluator,
   ...
   * for serialization
   */
  static final long serialVersionUID = 747878400873276317L;
  ...
  /**
   * Number of attributes in the training dataset
   */
  ...
  /**
   * by Suhel for CFS modifications
   */
  protected double m_vaThreshold = 0.5;

  /**
   * by Suhel
   */
  private double[] vaRanks = null;
...
/*  //TODO
    result.setValue(Field.AUTHOR, "F. Thabtah");
    result.setValue(Field.AUTHOR, "S. Hammoud");
    result.setValue(Field.YEAR, "2018");
    result.setValue(Field.TITLE, "TODO");
...
*/
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    String th = Utils.getOption('T', options);
    m_vaThreshold = Double.parseDouble(th);
  ...
  public void setVThreshold(double th) {
    this.m_vaThreshold = th;
  }

  public double getVThreshold() {
    return m_vaThreshold;
  }

  public String vThresholdTipText() {
    return "Va threshold ";
  }
  ...
  public void buildEvaluator(Instances data) throws Exception {
    Va va = new Va();
//    va.setVaFormula(getVaFormula());
    va.m_vaFormula = m_vaFormula;
    va.buildEvaluator(data);
    vaRanks = va.getAttributesRanks();

    buildEvaluatorLocal(data);
  }

  public void buildEvaluatorLocal(Instances data) throws Exception {
    ...
    if (!m_isNumeric) {
      ...
      if (m_debug) {
        System.err.println("Finished discretizing input dataset");
      }
    }
  ...
  public String toString() {
    ...
    if (m_trainInstances == null) {
      result.append("VaCFS subset evaluator has not been built yet\n");
    } else {
      result.append("\tVaCFS Subset Evaluator\n");

      if (m_missingSeparate) {
        result.append("\tTreating missing values as a separate value\n");
      }

      if (m_locallyPredictive) {
        result.append("\tIncluding locally predictive attributes\n");
      }

      result.append("\tVa Formula : " + m_vaFormula + "\n");
      result.append("Va threshold : " + m_vaThreshold + "\n");
    }
  ...
  public int[] postProcess(int[] attributeSet) throws Exception {
    //TODO check when not using locally predictive attributes
  ...
  public int[] postProcessLocal(int[] attributeSet) throws Exception {
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 00000 $");
  }
  ...
  public static void main(String[] args) {
    runEvaluator(new VaCfsEval(), args);
  }
}
